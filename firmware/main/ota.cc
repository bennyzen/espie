...
std::vector<long long> Ota::ParseVersion(const std::string& version) {
    std::vector<long long> versionNumbers;
    std::stringstream ss(version);
    std::string segment;

    while (std::getline(ss, segment, '.')) {
        versionNumbers.push_back(std::stoll(segment));
    }
...
bool Ota::IsNewVersionAvailable(const std::string& currentVersion, const std::string& newVersion) {
    std::vector<long long> current = ParseVersion(currentVersion);
    std::vector<long long> newer = ParseVersion(newVersion);
