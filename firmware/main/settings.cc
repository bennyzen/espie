...
Settings::~Settings() {
    if (nvs_handle_ != 0) {
        if (read_write_ && dirty_) {
            ...
            if (ret != ESP_OK) {
                ESP_LOGE(TAG, "nvs_commit failed for '%s': %s", ns_.c_str(), esp_err_to_name(ret));
            }
        }
...
std::string Settings::GetString(const std::string& key, const std::string& default_value) {
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "nvs_get_str failed for '%s/%s': %s", ns_.c_str(), key.c_str(), esp_err_to_name(ret));
        return default_value;
    }
...
void Settings::SetString(const std::string& key, const std::string& value) {
    if (read_write_) {
        ...
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "nvs_set_str failed for '%s/%s': %s", ns_.c_str(), key.c_str(), esp_err_to_name(ret));
            return;
        }
        dirty_ = true;
    } else {
...
void Settings::SetInt(const std::string& key, int32_t value) {
    if (read_write_) {
        ...
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "nvs_set_i32 failed for '%s/%s': %s", ns_.c_str(), key.c_str(), esp_err_to_name(ret));
            return;
        }
        dirty_ = true;
    } else {
...
void Settings::SetBool(const std::string& key, bool value) {
    if (read_write_) {
        ...
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "nvs_set_u8 failed for '%s/%s': %s", ns_.c_str(), key.c_str(), esp_err_to_name(ret));
            return;
        }
        dirty_ = true;
    } else {
...
void Settings::EraseKey(const std::string& key) {
    if (read_write_) {
        ...
        if (ret != ESP_OK && ret != ESP_ERR_NVS_NOT_FOUND) {
            ESP_LOGE(TAG, "nvs_erase_key failed for '%s/%s': %s", ns_.c_str(), key.c_str(), esp_err_to_name(ret));
        }
    } else {
        ESP_LOGW(TAG, "Namespace %s is not open for writing", ns_.c_str());
    }
...
void Settings::EraseAll() {
    if (read_write_) {
        ...
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "nvs_erase_all failed for '%s': %s", ns_.c_str(), esp_err_to_name(ret));
        }
    } else {
        ESP_LOGW(TAG, "Namespace %s is not open for writing", ns_.c_str());
    }
