...
void LcdDisplay::SetupUI() {
    ...
    // Left icons container (WiFi + server connection)
    ...
    server_label_ = lv_label_create(left_icons);
    ...
    lv_obj_set_style_margin_left(server_label_, lvgl_theme->spacing(2), 0);
    ...
    // No emoji in WeChat style — all screen space for chat
...
void LcdDisplay::SetChatMessage(const char* role, const char* content) {
    ...
    // Collapse system messages: update existing system bubble in place instead of
    // delete-and-recreate. lv_obj_del() from a non-LVGL task crashes in
    // lv_event_mark_deleted due to stale event pointers.
...
#if CONFIG_USE_WECHAT_MESSAGE_STYLE
    // In WeChat message style, hide emoji whenever there are chat messages
    uint32_t child_count = lv_obj_get_child_cnt(content_);
    if (child_count > 0) {
...
void LcdDisplay::CreateGlobalTopBar() {
    DisplayLockGuard lock(this);
    auto* theme = static_cast<LvglTheme*>(current_theme_);
    auto* icon_font = theme->icon_font()->font();

    // Build the top bar on lv_layer_top() — this layer persists across all
    // screen transitions, giving us a single status bar visible on every page.
    lv_obj_t* layer = lv_layer_top();

    lv_obj_t* bar = lv_obj_create(layer);
    lv_obj_set_size(bar, LV_HOR_RES, LV_SIZE_CONTENT);
    lv_obj_set_style_radius(bar, 0, 0);
    lv_obj_set_style_bg_opa(bar, LV_OPA_COVER, 0);
    lv_obj_set_style_bg_color(bar, theme->background_color(), 0);
    lv_obj_set_style_border_width(bar, 0, 0);
    lv_obj_set_style_pad_all(bar, 0, 0);
    lv_obj_set_style_pad_top(bar, theme->spacing(2), 0);
    lv_obj_set_style_pad_bottom(bar, theme->spacing(2), 0);
    lv_obj_set_style_pad_left(bar, theme->spacing(4), 0);
    lv_obj_set_style_pad_right(bar, theme->spacing(4), 0);
    lv_obj_set_flex_flow(bar, LV_FLEX_FLOW_ROW);
    lv_obj_set_flex_align(bar, LV_FLEX_ALIGN_SPACE_BETWEEN, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
    lv_obj_set_scrollbar_mode(bar, LV_SCROLLBAR_MODE_OFF);
    lv_obj_align(bar, LV_ALIGN_TOP_MID, 0, 0);

    // Left icons: WiFi + server connection
    ...
    // Replace the LvglDisplay label pointers so UpdateStatusBar() updates
    // these visible layer_top labels instead of the orphaned SetupUI ones.
    network_label_ = lv_label_create(left);
    lv_label_set_text(network_label_, "");
    lv_obj_set_style_text_font(network_label_, icon_font, 0);
    lv_obj_set_style_text_color(network_label_, theme->text_color(), 0);

    server_label_ = lv_label_create(left);
    lv_label_set_text(server_label_, "");
    lv_obj_set_style_text_font(server_label_, icon_font, 0);
    lv_obj_set_style_text_color(server_label_, lv_color_hex(0x888888), 0);
    lv_obj_set_style_margin_left(server_label_, theme->spacing(2), 0);
    ...
    mute_label_ = lv_label_create(right);
    lv_label_set_text(mute_label_, "");
    lv_obj_set_style_text_font(mute_label_, icon_font, 0);
    lv_obj_set_style_text_color(mute_label_, theme->text_color(), 0);

    battery_label_ = lv_label_create(right);
    lv_label_set_text(battery_label_, "");
    lv_obj_set_style_text_font(battery_label_, icon_font, 0);
    lv_obj_set_style_text_color(battery_label_, theme->text_color(), 0);
    lv_obj_set_style_margin_left(battery_label_, theme->spacing(2), 0);

    // Reset cached icon pointers so UpdateStatusBar() refreshes them
    battery_icon_ = nullptr;
    network_icon_ = nullptr;
    server_icon_ = nullptr;

    ESP_LOGI(TAG, "Global top bar created on lv_layer_top()");
}
