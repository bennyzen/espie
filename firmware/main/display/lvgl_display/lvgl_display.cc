...
void LvglDisplay::UpdateStatusBar(bool update_all) {
    ...
    // Clock display disabled — system time is unreliable without NTP
    ...
    // Update server connection icon
    {
        auto conn_state = Application::GetInstance().GetServerConnectionState();
        const char* new_icon = nullptr;
        lv_color_t color;

        if (conn_state == 2) { // connected
            new_icon = FONT_AWESOME_CLOUD;
            color = lv_color_hex(0x22c55e); // green
        } else if (conn_state == 1) { // connecting — blink
            static int blink_counter = 0;
            blink_counter++;
            new_icon = (blink_counter % 2 == 0) ? FONT_AWESOME_CLOUD : "";
            color = lv_color_hex(0xeab308); // amber
        } else { // disconnected
            new_icon = FONT_AWESOME_CLOUD_SLASH;
            color = lv_color_hex(0xef4444); // red
        }

        if (server_label_ != nullptr) {
            DisplayLockGuard lock(this);
            lv_label_set_text(server_label_, new_icon);
            lv_obj_set_style_text_color(server_label_, color, 0);
        }
    }
